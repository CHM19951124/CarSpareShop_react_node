...
const Menu = ({ history }) => (
    <div>
        <div className="w3-bar" style={{background:"black", color:"white"}}>
            <Link style={isActive(history, "/")} to="/" className="w3-bar-item w3-button ">Home</Link>
            <Link style={isActive(history, "/shop")} to="/shop" className="w3-bar-item w3-button">List</Link>

            {!isAuthenticated() && (
                <Fragment>
                    <Link style={isActive(history, "/signin")} to="/signin" className="w3-bar-item w3-button w3-right">
                        <i className='fas fa-user-circle' style={{fontSize:'20px'}}> Signin</i></Link>
                ...
                <Fragment>
                    ...
                    <div className="w3-dropdown-hover w3-right">
                        ...
                        <div className="w3-dropdown-content w3-bar-block w3-card-4">
                            <Link to={`/admin/products/${isAuthenticated().user._id}`} className="w3-bar-item w3-button">Advertise Manage</Link>
                            ...
                            <Link className="w3-bar-item w3-button">
                                <i className='fas fa-user-circle'
                                   onClick={() => signout(() => {history.push("/"); })} style={{fontSize:'20px',color:"black"}}> Sign Out</i></Link>
                        </div>
                    </div>
                </Fragment>
            )}
